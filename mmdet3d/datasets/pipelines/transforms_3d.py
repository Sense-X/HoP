...
class ObjectRangeFilter(object):
    ...
    def __init__(self, point_cloud_range, filter_adj_bbox=False):
        self.pcd_range = np.array(point_cloud_range, dtype=np.float32)
        self.filter_adj_bbox = filter_adj_bbox
    ...
    def __call__(self, input_dict):
        ...
        if self.filter_adj_bbox:
            input_dict = self.filter_adj(input_dict)

        return input_dict
    
    def filter_adj(self, input_dict):
        bev_range = self.pcd_range[[0, 1, 3, 4]]
        for idx, adj_gt_3d in enumerate(input_dict['adj_gt_3d']):
            gt_bboxes_3d, gt_labels_3d = adj_gt_3d
            mask = gt_bboxes_3d.in_range_bev(bev_range)
            gt_bboxes_3d = gt_bboxes_3d[mask]
            # mask is a torch tensor but gt_labels_3d is still numpy array
            # using mask to index gt_labels_3d will cause bug when
            ...
            # limit rad to [-pi, pi]
            gt_bboxes_3d.limit_yaw(offset=0.5, period=2 * np.pi)
            input_dict['adj_gt_3d'][idx] = [gt_bboxes_3d, gt_labels_3d]
        return input_dict
