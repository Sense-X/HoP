...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_adj_info(self, info, index):
        ...
        if not self.with_future_frame:
            max_id = min(index+1, len(self.data_infos)-1)
            ...
            max_id = min(index+len_future+1, len(self.data_infos)-1)
        ...
        if self.with_future_pred:
            if not self.data_infos[max_id]['scene_token'] == info[
                    'scene_token']:
                info_adj_list.append(info)
            else:
                info_adj_list.append(self.data_infos[max_id]) 
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            # convert pred bbox in ego coord to global coord
