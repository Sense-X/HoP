...
class FreeAnchor3DHead(Anchor3DHead):
    ...
    def __init__(self,
        ...
        self.num_pos_priors = num_pos_priors
    ...
    def loss(self,
        ...
        num_pos = 0
        positive_losses = []
        for idx, (anchors_, gt_labels_, gt_bboxes_, cls_prob_, bbox_preds_,
                dir_cls_preds_) in enumerate(
                    zip(anchors, gt_labels, gt_bboxes, cls_prob, bbox_preds,
                        dir_cls_preds)):
            anchors_, gt_labels_, gt_bboxes_ = anchors[idx], gt_labels[idx], gt_bboxes[idx]
            cls_prob_, bbox_preds_, dir_cls_preds_ = cls_prob[idx], bbox_preds[idx], dir_cls_preds[idx]
            ...
            pos_anchors = anchors_[pos_inds]
            matched_object_targets = self.bbox_coder.encode(
                matched_anchors,
                gt_bboxes_.unsqueeze(dim=1).expand_as(matched_anchors))

            num_gts = len(gt_labels_)
            matched_bbox_targets = gt_bboxes_.unsqueeze(dim=1).expand_as(pos_anchors).reshape(num_gts*self.num_pos_priors, -1)
            matched_cls_targets = gt_labels_.unsqueeze(dim=1).expand_as(pos_anchors[:,:,0]).reshape(num_gts*self.num_pos_priors)
        ...
        pos_anchors = pos_anchors.reshape(num_gts*self.num_pos_priors, -1)
        cls_weights = torch.ones_like(matched_bbox_targets)[:, 0].reshape(num_gts*self.num_pos_priors)
        #bbox_weights = bbox_weights.reshape(num_gts*self.num_pos_priors, -1)
        pos_priors = (pos_anchors, matched_cls_targets, cls_weights, matched_bbox_targets, bbox_weights, pos_anchors)
        losses = {
            'positive_bag_loss': positive_loss*self.loss_weight,
            'negative_bag_loss': negative_loss*self.loss_weight,
