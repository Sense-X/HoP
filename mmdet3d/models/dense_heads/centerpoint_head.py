...
class CenterHead(BaseModule):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        feature_map_size = torch.ceil(grid_size[:2] / self.train_cfg['out_size_factor'])
        feature_map_size = feature_map_size.type_as(grid_size)
    ...
    def get_tta_bboxes(self, preds_dicts_list, img_metas, img=None, rescale=False):
        """Generate bboxes from bbox head predictions.
        ...
        """
        mlvl_list = []
        for preds_dicts in preds_dicts_list:
            single_level_rec =  []
            for task_id, preds_dict in enumerate(preds_dicts):
                batch_heatmap = preds_dict[0]['heatmap'].sigmoid()
                ...
                temp = self.bbox_coder.decode(
                    ...
                    reg=batch_reg,
                    task_id=task_id)
                single_level_rec.append(temp)
            mlvl_list.append(single_level_rec)
        # merge mlvl_list
        mlvl_list = self.merge_mlvl_list(mlvl_list)
        # nms
        ret_list = self.get_tta_mlvl_bboxes(mlvl_list, img_metas, 
                                batch_size_list=[preds_dict[0]['heatmap'].shape[0]
                                                for preds_dict in preds_dicts_list[0]])
        return ret_list

    def merge_mlvl_list(self, mlvl_list):
        # len(preds_dict_list) -> len(task_id)
        ret_list = []
        for task_id in range(len(mlvl_list[0])):
            mlvl_single_task = {}
            for idx, single_level in enumerate(mlvl_list):
                single_temp = single_level[task_id][0]
                for key in single_temp.keys():
                    if key not in mlvl_single_task.keys():
                        mlvl_single_task[key] = single_temp[key]
                    else:
                        mlvl_single_task[key] = torch.cat(
                            [mlvl_single_task[key], single_temp[key]],
                            dim=0
                        )
            ret_list.append([mlvl_single_task])
        return ret_list

    def get_tta_mlvl_bboxes(self, mlvl_list, img_metas, batch_size_list):
        rets = []
        for task_id, temp in enumerate(mlvl_list):
            num_class_with_bg = self.num_classes[task_id]
            batch_size = batch_size_list[task_id]
            batch_reg_preds = [box['bboxes'] for box in temp]
            batch_cls_preds = [box['scores'] for box in temp]
            batch_cls_labels = [box['labels'] for box in temp]
            nms_type = self.test_cfg.get('nms_type')
