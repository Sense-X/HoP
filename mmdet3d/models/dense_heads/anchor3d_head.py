...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def _init_layers(self):
        ...
        self.conv_cls = nn.Conv2d(self.feat_channels, self.cls_out_channels, kernel_size=self.pred_kernel_size, padding=self.pred_kernel_size//2)
        self.conv_reg = nn.Conv2d(self.feat_channels,
                                  self.num_anchors * self.box_code_size, kernel_size=self.pred_kernel_size, padding=self.pred_kernel_size//2)
        if self.use_direction_classifier:
            self.conv_dir_cls = nn.Conv2d(self.feat_channels,
                                          self.num_anchors * 2, kernel_size=self.pred_kernel_size, padding=self.pred_kernel_size//2)
