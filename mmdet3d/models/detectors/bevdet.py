...
class BEVDet(CenterPoint):
    ...
    def extract_img_feat(self, img, img_metas, **kwargs):
        ...
        # pdb.set_trace() # check img.shape. why [x]_img[1:7]
    ...
    def aug_test(self,
        ...
        for i in range(len(img[0])):
            img_single = [img[j][i] for j in range(len(img))]
            img_feats, _, _ = self.extract_feat(
                points, img=img_single, img_metas=img_metas, **kwargs)
            outs = self.pts_bbox_head(img_feats)
            outs_list.append(outs)

        bbox_list = self.pts_bbox_head.get_tta_bboxes(
            outs_list, img_metas, rescale=rescale)
        bbox_pts = [
